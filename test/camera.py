...
class Camera(object):
    ...
    def __init__(self, env, near=0.1, far=100.0, image_size=336, dist=5.0, \
        ...
        #     theta = np.random.uniform(low=0.9, high=1.1) * np.pi
        #     phi = phi
        #     # dist = 4.8 + np.random.random()
        #     pos = np.array([dist*np.cos(phi)*np.cos(theta), \
        #             dist*np.cos(phi)*np.sin(theta), \
        #             dist*np.sin(phi)])
        #     # print(pos)
            
        #     b = pos[1] + np.random.random()*0.6 - 0.3
            
        #     pos[1] = b
        #     c = pos[2] + np.random.random()*0.4 - 0.2
        ...
        pos = np.array([dist*np.cos(phi)*np.cos(theta), \
                dist*np.cos(phi)*np.sin(theta), \
                dist*np.sin(phi)])
        # print(print('1',pos)pos)
            # print('2',pos)
        # pos = np.array([-3.54468498,-0.36440411,3.52577091])
    ...
    def compute_camera_XYZA(self, depth):
        ...
        out = np.zeros((996, 996, 4), dtype=np.float32)
        out[y, x, :3] = points
        out[y, x, 3] = 1
        return y, x, points, out
